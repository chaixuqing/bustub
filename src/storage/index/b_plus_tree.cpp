...
void BPLUSTREE_TYPE::StartNewTree(const KeyType &key, const ValueType &value) {
  ...
  Page *new_root_page_ptr = buffer_pool_manager_->NewPage(&new_root_page_id);
  IN_TREE_LEAF_PAGE_TYPE *new_root_page =
      reinterpret_cast<IN_TREE_LEAF_PAGE_TYPE *>(new_root_page_ptr->GetData());
  new_root_page->Init(new_root_page_id, INVALID_PAGE_ID, leaf_max_size_);
...
bool BPLUSTREE_TYPE::InsertIntoLeaf(const KeyType &key, const ValueType &value,
  ...
  IN_TREE_LEAF_PAGE_TYPE *leaf_page =
      reinterpret_cast<IN_TREE_LEAF_PAGE_TYPE *>(page->GetData());
  if (leaf_page->CheckDuplicated(key, comparator_)) {
    buffer_pool_manager_->UnpinPage(leaf_page->GetPageId(), false);
    return false;
  }
  leaf_page->Insert(key, value, comparator_);
  if (leaf_page->GetSize() >= leaf_max_size_) {
    IN_TREE_LEAF_PAGE_TYPE *new_leaf_page =
        Split<IN_TREE_LEAF_PAGE_TYPE>(leaf_page);
...
N *BPLUSTREE_TYPE::Split(N *node) {
  ...
  N *new_node = reinterpret_cast<N *>(new_page_ptr->GetData());
...
void BPLUSTREE_TYPE::InsertIntoParent(BPlusTreePage *old_node,
  ...
  if (old_node->IsRootPage()) {
    page_id_t new_root_page_id;
    Page *new_root_page_ptr = buffer_pool_manager_->NewPage(&new_root_page_id);
    IN_TREE_INTERNAL_PAGE_TYPE *new_root_page =
        reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>(
            new_root_page_ptr->GetData());
  ...
  IN_TREE_INTERNAL_PAGE_TYPE *parent_page_ptr =
      reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>(parent_page->GetData());
  parent_page_ptr->InsertNodeAfter(old_node->GetPageId(), key,
                                   new_node->GetPageId());
  if (parent_page_ptr->GetSize() >= internal_max_size_) {
    IN_TREE_INTERNAL_PAGE_TYPE *new_page_ptr =
        Split<IN_TREE_INTERNAL_PAGE_TYPE>(parent_page_ptr);
    parent_page_ptr->MoveHalfTo(new_page_ptr, buffer_pool_manager_);
    InsertIntoParent(parent_page_ptr, new_page_ptr->KeyAt(0), new_page_ptr,
                     transaction);
...
void BPLUSTREE_TYPE::Remove(const KeyType &key, Transaction *transaction) {
  ...
  transaction->AddIntoPageSet(nullptr);
  Page *page_ptr = FindLeafPage(key, false, 2, transaction);
  ...
  leaf_page_ptr->RemoveAt(index);
  page_ptr->SetDirty(true);
  if (leaf_page_ptr->GetSize() < leaf_page_ptr->GetMinSize()) {
    bool ret = CoalesceOrRedistribute<IN_TREE_LEAF_PAGE_TYPE>(leaf_page_ptr,
                                                              transaction);
    if (ret) {
...
bool BPLUSTREE_TYPE::CoalesceOrRedistribute(N *node, Transaction *transaction) {
  ...
  N *prev_page_ptr, *next_page_ptr;
  IN_TREE_INTERNAL_PAGE_TYPE *parent_page_ptr =
      reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>(parent_page->GetData());
  int index = parent_page_ptr->ValueIndex(node->GetPageId());
  if (index > 0) {
    prev_page_id = parent_page_ptr->ValueAt(index - 1);
    prev_page = buffer_pool_manager_->FetchPage(prev_page_id);
    prev_page_ptr = reinterpret_cast<N *>(prev_page->GetData());
    if (prev_page_ptr->GetSize() + node->GetSize() >=
        node->GetMaxSize()) {  // todo
      Redistribute(prev_page_ptr, node, 1);
      buffer_pool_manager_->UnpinPage(prev_page_id, true);
      buffer_pool_manager_->UnpinPage(parent_id, true);
      return false;
    }
  }
  if (index < parent_page_ptr->GetSize() - 1) {  // todo
    next_page_id = parent_page_ptr->ValueAt(index + 1);
    next_page = buffer_pool_manager_->FetchPage(next_page_id);
    next_page_ptr = reinterpret_cast<N *>(next_page->GetData());
    if (node->GetSize() + next_page_ptr->GetSize() >=
        node->GetMaxSize()) {  // todo
      Redistribute(next_page_ptr, node, 0);
  ...
  if (prev_page_id != INVALID_PAGE_ID) {
    bool res =
        Coalesce(&prev_page_ptr, &node, &parent_page_ptr, index, transaction);
    if (res) {
  ...
  bool res =
      Coalesce(&node, &next_page_ptr, &parent_page_ptr, index + 1, transaction);
 ...
 * coalesce or redistribute recursively if necessary. Using template N to
 * represent either internal page or leaf page.
...
bool BPLUSTREE_TYPE::Coalesce(
  ...
  if ((*node)->IsLeafPage()) {
    IN_TREE_LEAF_PAGE_TYPE *neighbor_page_ptr =
        reinterpret_cast<IN_TREE_LEAF_PAGE_TYPE *>((*neighbor_node));
    IN_TREE_LEAF_PAGE_TYPE *page_ptr =
        reinterpret_cast<IN_TREE_LEAF_PAGE_TYPE *>((*node));
    page_ptr->MoveAllTo(neighbor_page_ptr);
  } else {
    IN_TREE_INTERNAL_PAGE_TYPE *neighbor_page_ptr =
        reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>((*neighbor_node));
    IN_TREE_INTERNAL_PAGE_TYPE *page_ptr =
        reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>((*node));
    KeyType middle_key = (*parent)->KeyAt(index);
    page_ptr->MoveAllTo(neighbor_page_ptr, middle_key, buffer_pool_manager_);
  }
  (*parent)->Remove(index);
...
void BPLUSTREE_TYPE::Redistribute(N *neighbor_node, N *node, int index) {
  ...
  if (node->IsLeafPage()) {
    ...
    IN_TREE_LEAF_PAGE_TYPE *page_ptr =
        reinterpret_cast<IN_TREE_LEAF_PAGE_TYPE *>(node);
    if (index == 0) {
      neighbor_page_ptr->MoveFirstToEndOf(page_ptr);
      int node_index =
          parent_page_ptr->ValueIndex(neighbor_page_ptr->GetPageId());
      parent_page_ptr->SetKeyAt(node_index, neighbor_page_ptr->KeyAt(0));
    } else {
      neighbor_page_ptr->MoveLastToFrontOf(page_ptr);
      int node_index = parent_page_ptr->ValueIndex(page_ptr->GetPageId());
      parent_page_ptr->SetKeyAt(node_index, page_ptr->KeyAt(0));
    }
  } else {
    IN_TREE_INTERNAL_PAGE_TYPE *neighbor_page_ptr =
        reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>(neighbor_node);
    ...
    if (index == 0) {
      int node_index =
          parent_page_ptr->ValueIndex(neighbor_page_ptr->GetPageId());
      KeyType middle_key = parent_page_ptr->KeyAt(node_index);
      KeyType next_middle_key = neighbor_page_ptr->KeyAt(1);
      neighbor_page_ptr->MoveFirstToEndOf(page_ptr, middle_key,
                                          buffer_pool_manager_);

      parent_page_ptr->SetKeyAt(node_index, next_middle_key);
    } else {
      int node_index = parent_page_ptr->ValueIndex(page_ptr->GetPageId());
      KeyType middle_key = parent_page_ptr->KeyAt(node_index);
      KeyType next_middle_key =
          neighbor_page_ptr->KeyAt(neighbor_page_ptr->GetSize() - 1);
      neighbor_page_ptr->MoveLastToFrontOf(page_ptr, middle_key,
                                           buffer_pool_manager_);
      parent_page_ptr->SetKeyAt(node_index, next_middle_key);
    }
  }
 ...
 * case 1: when you delete the last element in root page, but root page
 * still has one last child case 2: when you delete the last element in
 * whole b+ tree
...
bool BPLUSTREE_TYPE::AdjustRoot(BPlusTreePage *old_root_node) {
  ...
  if (old_root_node->IsRootPage()) {
    ...
    IN_TREE_INTERNAL_PAGE_TYPE *old_root_page_ptr =
        reinterpret_cast<IN_TREE_INTERNAL_PAGE_TYPE *>(old_root_node);
    new_root_id = old_root_page_ptr->RemoveAndReturnOnlyChild();
 ...
 *****************************************************************************/
/*
 * Input parameter is void, find the leaftmost leaf page first, then
...
INDEXITERATOR_TYPE BPLUSTREE_TYPE::begin() {
  root_id_latch_.RLock();
  Page *left_most_page = FindLeafPage(KeyType(), true, 0);
  return INDEXITERATOR_TYPE(left_most_page, 0, buffer_pool_manager_);
}

/*
 * Input parameter is low key, find the leaf page that contains the input
 * key first, then construct index iterator
 * @return : index iterator
 */
INDEX_TEMPLATE_ARGUMENTS
INDEXITERATOR_TYPE BPLUSTREE_TYPE::Begin(const KeyType &key) {
  root_id_latch_.RLock();
  Page *leaf_page = FindLeafPage(key, false, 0);
  IN_TREE_LEAF_PAGE_TYPE *leaf_ptr =
      reinterpret_cast<IN_TREE_LEAF_PAGE_TYPE *>(leaf_page->GetData());
...
INDEX_TEMPLATE_ARGUMENTS
void BPLUSTREE_TYPE::ReleaseLatchQueue(
    Transaction *transaction, int mode) {  // not release root_id_latch_
  if (transaction == nullptr) {
    return;
  }
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
bool BPLUSTREE_TYPE::CheckSafe(BPlusTreePage *tree_ptr, int mode) {
  if (mode == 0 || mode == 3) {
    return true;
  }
  ...
  if (mode == 2) {
    if (tree_ptr->IsRootPage() && tree_ptr->IsLeafPage() &&
        tree_ptr->GetSize() > 1) {
      return true;
    }
    if (tree_ptr->IsRootPage() && !tree_ptr->IsLeafPage() &&
    ...
    if (!tree_ptr->IsRootPage() &&
        tree_ptr->GetSize() > tree_ptr->GetMinSize()) {
      return true;
    }
  }
  return false;
...
/*
 * Update/Insert root page id in header page(where page_id = 0, header_page
 * is defined under include/page/header_page.h) Call this method everytime
